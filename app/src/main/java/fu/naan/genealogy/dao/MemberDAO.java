...
//        values.put(DatabaseHandler.MEMBER_COLUMN_DOD, member.getDOD().toString());
        values.put(DatabaseHandler.MEMBER_COLUMN_AVATAR, member.getAvatar().toString());
...
//        values.put(DatabaseHandler.MEMBER_COLUMN_FATHER, member.getFatherID());
    ...
    public int deleteByMemberID(int memberID) {
        int result = -1;
        SQLiteDatabase db = databaseHandler.getWritableDatabase();
        result = db.delete(DatabaseHandler.TABLE_MEMBER, DatabaseHandler.MEMBER_COLUMN_ID + " = ?",
                    new String[]{String.valueOf(memberID)});
        db.close();
        return result;
    }
    public void update(Member member) {
        SQLiteDatabase mDb= databaseHandler.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(DatabaseHandler.MEMBER_COLUMN_ID,member.getMemberID());
        cv.put(DatabaseHandler.MEMBER_COLUMN_NAME,member.getMemberName());
        cv.put(DatabaseHandler.MEMBER_COLUMN_DOB,Common.dateFormat(member.getDOB()));
        cv.put(DatabaseHandler.MEMBER_COLUMN_GENDER,member.getGender());
        mDb.update(DatabaseHandler.TABLE_MEMBER,cv,DatabaseHandler.MEMBER_COLUMN_ID + "= ?",new String[]{Integer.toString(member.getMemberID())});
        mDb.close();
    }
    ...
    private ArrayList<Member> select(String query) {
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast()) {
                ...
                String avatar = cursor.getString(cursor.getColumnIndex(DatabaseHandler.MEMBER_COLUMN_AVATAR));
                members.add(new Member(id,name, Common.stringToDate(dob),gender,
                        avatar==null?null:Common.stringToBitMap(avatar)));
                cursor.moveToNext();
            }
        }
        cursor.close();
    ...
    public ArrayList<Member> selectByNodeId(int nodeId) {
        String query = "SELECT * FROM " + DatabaseHandler.TABLE_MEMBER_IN_NODE + " LEFT JOIN "
                ...
                + " WHERE " + DatabaseHandler.TABLE_MEMBER_IN_NODE + "." + DatabaseHandler.MEMBER_IN_NODE_COLUMN_NODE_ID + " = " + nodeId;
        return select(query);
    }
